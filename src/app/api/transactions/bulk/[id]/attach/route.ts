...
export async function POST(req: Request, context: any) {
  ...
  const txs = await prisma.transaction.findMany({ where: { transactionBulkId: bulkId }, select: { id: true } });
  if (!txs?.length) {
  ...
  try {
    await prisma.$transaction(async (p: any) => {
      for (const tx of txs) {
        await p.transaction.update({ where: { id: tx.id }, data: { attachment: { connect: { id: attachmentId } } } });
      }
    });
  } catch (e: any) {
    return NextResponse.json({ error: 'Fehler beim Speichern des Belegs' }, { status: 500 });
  }
  return NextResponse.json({ bulkId, attachmentId, count: txs.length }, { status: 201 });
}
